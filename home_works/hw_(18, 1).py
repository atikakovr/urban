...
Задание:
Один из работников Urban'a огромный фанат криптовалюты и всё что с ней связано. Говорят, он готов продать целый Эфир
 лишь за то, чтобы у него была небольшая программка, которая поможет ему распределять финансы между криптовалютами входящими в топ-100
 капитализации.
Самые актуальные данные по криптовалютам можно найти на CoinMarketCap
.
На домашней странице этого сайта и находится топ-100
 всех криптовалют.
...
Для парсинга
 и запросов
 разрешено использовать любую из перечисленных библиотек: requests
, selenium
, beautifulsoup
, scrapy
.
Ваш код должен выполнять следующий функционал:
Считывать данные с сайта: название криптовалюты
,
 текущая рыночная капитализация
Записывать данные в CSV файл в следующем порядке: название криптовалюты, текущая рыночная капитализация, процент от общей капитализации топ-100
 криптовалют
.
...
-год.
Запись в CSV
 файл должна быть в следующем формате (цифры случайны):
