"""
2024/01/15 00:00|Домашнее задание по теме "Логирование"
Цель задания:
...
Задание:
Вам было поручено отобрать сайты которые не работают без VPN (
не удаётся подключится
), а так же те, ответ на запрос к которым не содержит нужно информации (
отличных от 200
).
...
.
Все сайты к которым возможно сделать запрос и получить ответ 200
 должны записываться в отдельный лог-файл (
success_responses.log
).
Все сайты к которым возможно сделать запрос и получить ответ отличный от 200
 должны записываться в отдельный лог-файл (
bad_responses.log
).
Все сайты к которым возможно сделать запрос, но получить ответ от них невозможно
, должны записываться в отдельный лог-файл (
blocked_responses.log
).
Для того, чтобы узнать у объекта Response
 код используйте его атрибут status_code
.
...
ПОДСКАЗКА: классы исключений запросов находятся в rq.exceptions
. Обрабатывайте их конструкцией try, except
.
