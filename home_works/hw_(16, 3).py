...
Задание:
Как мы все знаем айтишники любят путешествовать, а география студентов Urban'а
 очень большая.
...
Напишите функцию
 write_holiday_cities(first_letter)
, которая: Принимает параметром первую букву имени человека - first_letter
.
Функция должна:
Получить данные из travel_notes.csv и записать в holiday.csv
:
В каких городах студенты с именем на first_letter
 уже были.
Какие города студенты с именем на first_letter хотят посетить.
В каких городах студенты с именем на first_letter
 ещё не были.
Какой первый город эти студенты посетят (
в алфавитном порядке
)
Файлы для задачи - https://github.com/yanchuki/StudentsHoliday.git
Пример работы:
Входные данные:
Текстовый файл travel_notes.csv
, содержит данные о студентах и городах, в которых они были и хотят посетить.
Например:
...
Ввод: L
Выходные данные
Все города, записанные в holiday.csv
 должны быть предварительно отсортированы в алфавитном порядке.
...
Примечание:
Кроме буквы "L"
 достаточным будет проверка результата с буквой "R"
:
Посетили: Lyubertsy, Nizhny Tagil, Odintsovo Хотят посетить: Kiev, Perm Никогда не были в: Kiev, Perm Следующим городом будет: Kiev
